...
**用途**：自主執行深度學習實驗迴圈 -- 修改程式碼、訓練、評估、保留或捨棄，無限重複
...
技能實作了受 [karpathy/autoresearch](https://github.com/karpathy/autoresearch) 啟發的自主實驗迴圈：

1. **設定**：確認範圍（目標檔案、指標、時間預算），建立實驗分支，跑 baseline
2. **無限迴圈**：
   - 修改目標檔案，嘗試實驗性想法
   - Git commit 變更
   - 執行實驗（固定時間預算，如 5 分鐘）
   - 從 log 擷取指標
   - 指標改善則**保留**（推進分支），否則**捨棄**（git reset）
   - 記錄結果至 `results.tsv`
   - 無限重複直到手動中斷
3. **分析**：session 後用 Jupyter notebook 視覺化進度、結果分佈和 top hits

**核心設計原則**：
- **單一可修改檔案**：agent 只編輯一個檔案，其他全部唯讀
- **固定時間預算**：每次實驗相同的 wall-clock 時長，確保公平比較
- **單一指標**：一個標量決定保留或捨棄，不做多目標平衡
- **簡潔準則**：同等性能下更簡單的程式碼也算贏
- **永不停止**：agent 無限運行直到使用者中斷

**結構**：
- `SKILL.md` - 主要技能定義和工作流程
- `references/experiment-protocol.md` - 詳細實驗迴圈協議，含 crash 處理和決策規則
- `references/analysis-template.md` - session 後分析的 Jupyter notebook 模板

**目標用戶**：執行迭代實驗的深度學習研究學者
...
| **研究** | paper, autoresearch | 可用 | 深度學習研究學者 |
...
2. **autoresearch** - 整夜自主跑實驗，睡醒看結果
3. **scientific-brainstorming** - 用於生成研究假設和實驗構想
